(function() {
	'use strict';
	var controllerId = 'detailMonthPlanTTXDController';
	
	angular.module('MetronicApp').controller(controllerId, detailMonthPlanTTXDController);
	
	function detailMonthPlanTTXDController($scope, $rootScope, $timeout, gettextCatalog, 
			kendoConfig, $kWindow,detailMonthPlanTTXDService,$filter,
		...
		vm.String="Quản lý công trình > Quản lý kế hoạch > Kế hoạch tháng chi tiết trung tâm xây dựng";
		vm.detailMonthPlan={};
	    ...
	    initTab2TTXDFunction($scope, $rootScope, $timeout, gettextCatalog,
						...
						transport: {
							...
							parameterMap: function (options, type) {
	                              // vm.detailMonthPlanSearch.employeeId =
									...
									vm.detailMonthPlanSearch.type = "2";
    ...
    vm.exportExcelGrid= function(){
    	...
    	obj.pageSize=null;
    	obj.type="2";
    	detailMonthPlanTTXDService.doSearch(obj).then(function(data){
    		CommonService.exportFile(vm.detailMonthPlanGrid, data.data, vm.listRemove, vm.listConvert, "Kế hoạch tháng chi tiết TTXD");
    	});
    }
	...
	function updateRegistry(dataItem){
        return Restangular.all("DetailMonthPlanRsService/checkPermissionsRegistry").post(dataItem).then(function (d) {
            ...
            confirm('Bạn thật sự muốn trình ký bản ghi đã chọn?', function () {
            	detailMonthPlanTTXDService.updateRegistry(dataItem).then(
                    function (d) {
    ...
    vm.exportExcelTab1= function(){
    	function displayLoading(target) {
			...
			setTimeout(function(){
				...
				return Restangular.all("DetailMonthPlanOSRsService/exportGiaoChiTietTTXD").post(vm.searchForm).then(function (d) {
		            var data = d.plain();
...
vm.handleCheck=handleCheck;
		...
		function add(){
            ...
            return Restangular.all("DetailMonthPlanOSRsService/checkPermissionsAdd").post().then(function (d) {
                ...
                vm.showDetail = true;
                vm.sysGroupCode = '';
                vm.String = "Quản lý công trình > Quản lý kế hoạch > Thêm mới kế hoạch tháng chi tiết TTXD";
                var year = (new Date()).getFullYear();
                ...
                detailMonthPlanTTXDService.getSequence({}).then(function (seq) {
                    vm.seq = seq;
		...
		function edit(dataItem){
            ...
            initTab2Table();
            detailMonthPlanTTXDService.getById(dataItem.detailMonthPlanId).then(function(result){
                vm.String="Quản lý công trình > Quản lý kế hoạch > Chỉnh sửa kế hoạch tháng chi tiết TTXD";
                vm.showDetail = true;
...
//      hungnx 20180619 end
		vm.save= function() {
		    ...
		    if (validateTotalMonthDetail()) {
                ...
                data.lstMonthQuantity=vm.targetGrid.dataSource.data();
                data.type = "2";
                
                if(data.listTC.length==0){
                	toastr.warning("Chưa có dữ liệu trong bảng Giao chỉ tiêu tháng");
                	vm.disabled=false;
                	return;
                }
//              hungnx 20180619 start
...
//              hungnx 20180619 end
		        if (vm.detailMonthPlan.detailMonthPlanId == null || vm.detailMonthPlan.detailMonthPlanId == '') {
		            ...
		            detailMonthPlanTTXDService.createdetailMonthPlan(data).then(function (result) {
		                if (result.error) {	
		                    $('#year').focus();
...
//                      hungnx 20180619 end
 //                       vm.cancelAdd()
		            ...
		            detailMonthPlanTTXDService.updatedetailMonthPlan(data).then(function (result) {
		                if (result.error) {
		                    $('#year').focus();
		                    toastr.error(result.error);
//	                      hungnx 20180619 start
...
// var dataItem = grid.dataSource.getByUid(uid);
                confirm('Bạn thật sự muốn xóa bản ghi đã chọn?', function () {
                    detailMonthPlanTTXDService.remove(dataItem).then(
                        function (data) {
		...
		vm.gridColumnShowHideFilter = function (item) { 
            ...
            vm.exportpdf= function(){
            	var obj={};
            	detailMonthPlanTTXDService.exportpdf(obj);
            }
