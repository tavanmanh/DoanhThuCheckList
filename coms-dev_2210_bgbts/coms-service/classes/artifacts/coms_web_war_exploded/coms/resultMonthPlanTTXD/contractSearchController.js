/* Modal Controller */
MetronicApp.controller('contractSearchController', ['$scope', 'dataTree', 'caller', 'modalInstance1', 'gridOptions', 'popupId', 'isMultiSelect', 'CommonService', 'htmlCommonService', 'SearchService', 'PopupConst', 'RestEndpoint', '$localStorage', '$rootScope', 'Constant', 'gettextCatalog', '$timeout', 
	function ($scope, dataTree, caller, modalInstance1, gridOptions, popupId, isMultiSelect, CommonService, htmlCommonService, SearchService, PopupConst, RestEndpoint, $localStorage, $rootScope, Constant, gettextCatalog, $timeout) {
		...
		$(document).ready(function () {
			fillTable();
			$timeout(function (){angular.element('#contractCode').focus();}, 500);
		});
			...
			$scope.gridOptions = kendoConfig.getGridOptions({
				...
				dataSource: {
					...
					transport: {
						read: {
							...
							url: Constant.BASE_SERVICE_URL + "DetailMonthPlanOSRsService/doSearchContractByPopup",
						...
						parameterMap: function (options, type) {
							...
							if (!!$scope.searchGrid)
								obj = {
									cntContractCode: $scope.searchGrid.code,
								}
				...
				}, 
				{
					...
					template:
						...
						'			<i id="#=cntContractCode#" ng-click=save(dataItem) class="fa fa-check color-green" aria-hidden="true"></i> ' +
						'		</a>'
						+ '</div>',
					width: "15%",
					field: "stt"
