(function() {
	'use strict';
	var controllerId = 'totalMonthPlanOSController';
	
	angular.module('MetronicApp').controller(controllerId, totalMonthPlanOSController);
	
	function totalMonthPlanOSController($scope, $rootScope, $timeout, gettextCatalog, 
			kendoConfig, $kWindow,totalMonthPlanOSService,$filter,
        ...
        vm.nextStep2 = function () {
            ...
            var targetGrid = $("#targetGrid").data("kendoGrid").dataSource._data;
            if(targetGrid.length > 0){
            }else{
            	if(vm.totalMonthPlan.totalMonthPlanId != null || vm.totalMonthPlan.totalMonthPlanId != undefined){
            		kendo.ui.progress($(".tab-content"),true);
            		totalMonthPlanOSService.getListTmpn(vm.totalMonthPlan.totalMonthPlanId).then(function (data) {
                        if(data){
                    	...
                    	$("#targetGrid").data("kendoGrid").dataSource.page(1);
                        }, function (error) {
        ...
        vm.totalMonthPlanOSSearch = {
            status: 1
        };
        vm.String = "Quản lý công trình > Quản lý kế hoạch > Kế hoạch tháng tổng thể ngoài OS";
        vm.totalMonthPlan = {

        };
        vm.cancelListMonth = function () {
            vm.totalMonthPlanOSSearch.monthList = [];
        }
        vm.cancelListStatus = function () {
            vm.totalMonthPlanOSSearch.signStateList = [];
        }
        vm.cancelListYear = function () {
            vm.totalMonthPlanOSSearch.yearList = [];
        }
        vm.dataRropFile = [];
        function loadFileData(){
            totalMonthPlanOSService.getFileAppendixParam().then(function(result){
        ...
        initTab2OSFunctionTotal($scope, $rootScope, $timeout, gettextCatalog,
...
//            initTab2Table();
                ...
                toolbar: [
                    {
                        ...
                        '<label ng-repeat="column in vm.totalMonthPlanOSGrid.columns.slice(1,vm.totalMonthPlanOSGrid.columns.length)| filter: vm.gridColumnShowHideFilter">' +
                ...
                dataSource: {
                    ...
                    transport: {
                        read: {
                            ...
                            url: Constant.BASE_SERVICE_URL + "totalMonthPlanOSRsService/doSearch",
                            contentType: "application/json; charset=utf-8",
                            type: "POST"
                        },
                        parameterMap: function (options, type) {
                            // vm.totalMonthPlanOSSearch.employeeId =
                            // Constant.user.srvUser.catEmployeeId;
                            vm.totalMonthPlanOSSearch.page = options.page
                            vm.totalMonthPlanOSSearch.pageSize = options.pageSize

                            return JSON.stringify(vm.totalMonthPlanOSSearch)
		...
		//	var data = vm.totalMonthPlanOSGrid.dataSource.data();
		//		CommonService.exportFile(vm.totalMonthPlanOSGrid,data,vm.listRemove,vm.listConvert);
		//}
        ...
        vm.exportFile= function(){
        	function displayLoading(target) {
        	      ...
        	      setTimeout(function(){
        	    	  ...
        	    	  return Restangular.all("totalMonthPlanOSRsService/exportTotalMonthPlan").post(vm.totalMonthPlanOSSearch).then(function (d) {
        	                var data = d.plain();
...
vm.listConvert1 = [{
    field: "status"
        ...
        vm.exportFileChiTieu= function(){
			function displayLoading(target) {
				...
				setTimeout(function(){
					...
					return Restangular.all("totalMonthPlanOSRsService/exportDetailTargetTotalMonthPlan").post(vm.target).then(function (d) {
						var data = d.plain();
						window.location = Constant.BASE_SERVICE_URL + "fileservice/downloadFileATTT?" + data.fileName;
						kendo.ui.progress(element, false);
					}).catch(function (e) {
        ...
        vm.exportFileDamBaoNguonViec= function() {
        	function displayLoading(target) {
				...
				setTimeout(function(){
					...
					return Restangular.all("totalMonthPlanOSRsService/exportDetailSourceTotalMonthPlan").post(vm.source).then(function (d) {
		                var data = d.plain();
		                window.location = Constant.BASE_SERVICE_URL + "fileservice/downloadFileATTT?" + data.fileName;
		                kendo.ui.progress(element, false);
		            }).catch(function (e) {
        ...
        vm.exportFileForceMaintain= function(){
        	function displayLoading(target) {
				...
				setTimeout(function(){
					...
					return Restangular.all("totalMonthPlanOSRsService/exportDetailForceMaintainTotalMonthPlan").post(vm.forceMaintain).then(function (d) {
		                var data = d.plain();
		                window.location = Constant.BASE_SERVICE_URL + "fileservice/downloadFileATTT?" + data.fileName;
		                kendo.ui.progress(element, false);
		            }).catch(function (e) {
        	...
        	function displayLoading(target) {
				...
				setTimeout(function(){
					...
					vm.totalMonthPlanId= vm.totalMonthPlanId==null?-1:vm.totalMonthPlanId
				            return Restangular.all("totalMonthPlanOSRsService/exportDetailBTSGTotalMonthPlan").post(vm.totalMonthPlanId).then(function (d) {
				                var data = d.plain();
				                window.location = Constant.BASE_SERVICE_URL + "fileservice/downloadFileATTT?" + data.fileName;
				                kendo.ui.progress(element, false);
				            }).catch(function (e) {
        ...
        vm.exportFileForceNew= function(){
        	function displayLoading(target) {
				...
				setTimeout(function(){
					...
					vm.totalMonthPlanId= vm.totalMonthPlanId==null?-1:vm.totalMonthPlanId
				            return Restangular.all("totalMonthPlanOSRsService/exportDetailForceNewTotalMonthPlan").post(vm.totalMonthPlanId).then(function (d) {
				                var data = d.plain();
				                window.location = Constant.BASE_SERVICE_URL + "fileservice/downloadFileATTT?" + data.fileName;
				                kendo.ui.progress(element, false);
				            }).catch(function (e) {
        ...
        vm.exportFileMaterial= function(){
        	function displayLoading(target) {
				...
				setTimeout(function(){
					...
					return Restangular.all("totalMonthPlanOSRsService/exportDetailMaterialTotalMonthPlan").post(vm.material).then(function (d) {
		                var data = d.plain();
		                window.location = Constant.BASE_SERVICE_URL + "fileservice/downloadFileATTT?" + data.fileName;
		                kendo.ui.progress(element, false);
		            }).catch(function (e) {
        ...
        vm.exportFileFinance= function(){
        	function displayLoading(target) {
				...
				setTimeout(function(){
					...
					return Restangular.all("totalMonthPlanOSRsService/exportDetailFinanceTotalMonthPlan").post(vm.finance).then(function (d) {
		                var data = d.plain();
		                window.location = Constant.BASE_SERVICE_URL + "fileservice/downloadFileATTT?" + data.fileName;
		                kendo.ui.progress(element, false);
		            }).catch(function (e) {
        ...
        vm.exportFileContract= function(){
        	function displayLoading(target) {
				...
				setTimeout(function(){
					...
					return Restangular.all("totalMonthPlanOSRsService/exportDetailContractTotalMonthPlan").post(vm.contract).then(function (d) {
		                var data = d.plain();
		                window.location = Constant.BASE_SERVICE_URL + "fileservice/downloadFileATTT?" + data.fileName;
		                kendo.ui.progress(element, false);
		            }).catch(function (e) {
		...
		function refreshGrid(d) {
			var grid = vm.totalMonthPlanOSGrid;
			if(grid){
        ...
        function add() {
            ...
            return Restangular.all("totalMonthPlanOSRsService/checkPermissionsAdd").post().then(function (d) {
                if(d.error){
  ...
  //          vm.totalMonthPlan.name = 'Kế hoạch tổng thể thực hiện tháng ' + vm.totalMonthPlan.month + "/" + year
            totalMonthPlanOSService.getSequence({}).then(function (seq) {
                vm.seq = seq;
                if(vm.totalMonthPlan.month>=10){
...
//            },800)
            ...
            totalMonthPlanOSService.getById(dataItem.totalMonthPlanId).then(function (data) {
                if(data){
                	kendo.ui.progress($(".tab-content"),false);
                }
                initTab2Table();
            	vm.totalMonthPlan = data;
//                    refreshappendixGrid(data); hoangnh cmt
...
//                kendo.ui.progress($(".tab-content"),false);
                ...
                return Restangular.all("totalMonthPlanOSRsService/checkPermissionsUpdate").post().then(function (d) {
                    if(d==2){
...
//            },100);
        ...
        function copy(dataItem) {
            return Restangular.all("totalMonthPlanOSRsService/checkPermissionsCopy").post().then(function (d) {
                if(d.error){
                    toastr.error(d.error);
                    return;
                }
                ...
                vm.action = "Thêm mới kế hoạch tháng";
                vm.showDetail = true;
                totalMonthPlanOSService.getByIdCopy(dataItem.totalMonthPlanId).then(function (data) {
                    vm.totalMonthPlan = data;
                    refreshAllGridTab2(data);
                    refreshGrid(data.listAttachFile, vm.attachFileGrid);
...
//                        }
                    ...
                    totalMonthPlanOSService.createTotalMonthPlan(data).then(function (result) {
                        if (result.error) {
                        ...
                        vm.cancel();
                        vm.totalMonthPlanOSGrid.dataSource.page(1);
                        doSearch();
                        vm.loadingSave = false;
                    }, function (errResponse) {
                    ...
                    if(vm.attachFileGrid!=undefined && vm.attachFileGrid.dataSource!=undefined)
                        ...
                        for(var i=0 ; i < vm.totalMonthPlan.tmpnTargetDTOList.length ; i++){
                        	delete vm.totalMonthPlan.tmpnTargetDTOList[i].fwmodelId;
                        }
                    delete vm.totalMonthPlan.fwmodelId;
                    totalMonthPlanOSService.updateTotalMonthPlan(vm.totalMonthPlan).then(function (result) {
                        if (result.error) {
                            toastr.error(result.error);
                            vm.loadingSave = false;
                            return;
        ...
        function remove(dataItem) {
            return Restangular.all("totalMonthPlanOSRsService/checkPermissionsDelete").post().then(function (d) {
                if(d.error){
                    toastr.error(d.error);
                    return;
                }
                ...
                confirm('Bạn thật sự muốn xóa bản ghi đã chọn?', function () {
                	delete dataItem.fwmodelId;
                	delete dataItem.filePathError;
                    totalMonthPlanOSService.remove(dataItem).then(
                        function (d) {
                            ...
                            vm.totalMonthPlanOSGrid.dataSource.page(1);
                            vm.totalMonthPlanOSGrid.refresh();
                            var sizePage = $("#totalMonthPlanOSGrid").data("kendoGrid").dataSource.total();
                            var pageSize = $("#totalMonthPlanOSGrid").data("kendoGrid").dataSource.pageSize();
                            if (sizePage % pageSize === 1) {
                                var currentPage = $("#totalMonthPlanOSGrid").data("kendoGrid").dataSource.page();
                                if (currentPage > 1) {
                                    $("#totalMonthPlanOSGrid").data("kendoGrid").dataSource.page(currentPage - 1);
                                }
                            }
                            /* $("#totalMonthPlanOSGrid").data('kendoGrid').dataSource.read();
                             $("#totalMonthPlanOSGrid").data('kendoGrid').refresh();*/
...
//                vm.totalMonthPlanOSGrid.dataSource.page(1);
        ...
        vm.showHideColumn = function (column) {
            if (angular.isUndefined(column.hidden)) {
                vm.totalMonthPlanOSGrid.hideColumn(column);
            } else if (column.hidden) {
                vm.totalMonthPlanOSGrid.showColumn(column);
            } else {
                vm.totalMonthPlanOSGrid.hideColumn(column);
            }
        ...
        vm.exportpdf = function () {
            var obj = {};
            totalMonthPlanOSService.exportpdf(obj);
        }
        ...
        function sendToSign(dataItem) {
            return Restangular.all("totalMonthPlanOSRsService/checkPermissionsRegistry").post().then(function (d) {
                if(d.error){
                    toastr.error(d.error);
                    return;
                }
        ...
        function updateRegistry(dataItem) {
            return Restangular.all("totalMonthPlanOSRsService/checkPermissionsRegistry").post().then(function (d) {
                ...
                confirm('Bạn thật sự muốn trình ký bản ghi đã chọn?', function () {
                	delete dataItem.fwmodelId;
                	totalMonthPlanOSService.updateRegistry(dataItem).then(
                        function (d) {
                            if (d != undefined && d.error) {
                                toastr.error(d.error);
                                return;
