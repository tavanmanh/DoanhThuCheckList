/* Modal Controller */
MetronicApp.controller('staffConfirmSearchController', ['$scope', 'dataTree', 'caller', 'modalInstance1', 'gridOptions', 'popupId', 'isMultiSelect', 'CommonService', 'htmlCommonService', 'SearchService', 'PopupConst', 'RestEndpoint', '$localStorage', '$rootScope', 'Constant', 'gettextCatalog', '$timeout', 
	function ($scope, dataTree, caller, modalInstance1, gridOptions, popupId, isMultiSelect, CommonService, htmlCommonService, SearchService, PopupConst, RestEndpoint, $localStorage, $rootScope, Constant, gettextCatalog, $timeout) {
		...
		$(document).ready(function () {
			fillTable();
			$timeout(function (){angular.element('#staffName').focus();}, 500);
		});
			...
			$scope.gridOptions = kendoConfig.getGridOptions({
				...
				dataSource: {
					...
					transport: {
						read: {
							...
							url: Constant.BASE_SERVICE_URL + "configStaffTangentRestService/catProvince/doSearchStaffByConfigProvinceId",
						...
						parameterMap: function (options, type) {
							...
							if (!!$scope.searchGrid)
								obj = {
									staffName: $scope.searchGrid.code,
								}
							obj.type = "1";
							obj.catProvinceId = caller.provinceId;
				...
				}, 
				{
					...
					template:
						...
						'			<i id="#=staffCode#" ng-click=save(dataItem) class="fa fa-check color-green" aria-hidden="true"></i> ' +
						'		</a>'
						+ '</div>',
					width: "15%",
					field: "stt"
		...
		function save(dataItem) {
			if (dataItem) {
				if($scope.popupId=='giaiPhapNext'){
					caller.onSaveStaffGiaiPhap(dataItem);
					htmlCommonService.dismissPopup();
				} else if($scope.popupId=='tiepXucNext') {
					caller.onSaveStaffTiepXuc(dataItem);
					htmlCommonService.dismissPopup();
				} else {
					caller.onSaveStaff(dataItem);
					htmlCommonService.dismissPopup();
				}
			}
		}
