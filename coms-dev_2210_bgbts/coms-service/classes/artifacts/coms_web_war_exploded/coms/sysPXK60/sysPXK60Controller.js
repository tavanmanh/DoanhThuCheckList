...
//        }
        ...
        vm.workItemSearch = {};
        vm.listDataObj = [];
        vm.String = "Quản lý công trình > Báo cáo > Báo cáo PXK chậm hoàn công 60 ngày";
                ...
                toolbar: [
                    {
                        ...
                        template:
                            ...
                            '<label ng-repeat="column in vm.workItemGridSixty.columns.slice(1,vm.workItemGridSixty.columns.length)| filter: vm.gridColumnShowHideFilter">' +
                ...
                dataSource: {
                    ...
                    schema: {
                        total: function (response) {
                            $("#appCountWorkItem60").text("" + response.total);
                            vm.countWorkItem60 = response.total;
                    ...
                    transport: {
                        read: {
                            ...
                            url: Constant.BASE_SERVICE_URL + "rpQuantityService/doSearchSysPXK60",
...
//HuyPQ-25/08/2018-edit-start
        vm.exportWorkItemFile = function () {
            ...
            displayLoading("#workItemGridSixty");
        }
//HuyPQ-edit-end
        ...
        function getConstructionTask() {
            var list = [];
            var data = vm.workItemGridSixty.dataSource._data;
            vm.workItemGridSixty.table.find("tr").each(function (idx, data) {
                var row = $(data);
                var checkbox = $('[name="gridcheckbox"]', row);
                if (checkbox.is(':checked')) {
                    var tr = vm.workItemGridSixty.select().closest("tr");
                    var dataItem = vm.workItemGridSixty.dataItem(data);
        ...
        vm.showHideWorkItemColumn = function (column) {
            if (angular.isUndefined(column.hidden)) {
                vm.workItemGridSixty.hideColumn(column);
            } else if (column.hidden) {
                vm.workItemGridSixty.showColumn(column);
            } else {
                vm.workItemGridSixty.hideColumn(column);
            }
        ...
        covenantSysPxk60Function($scope, $rootScope, $timeout, gettextCatalog, $filter,
...
// 8.2 Search SysGroup
        ...
        vm.exportFileCons = function exportFile() {
            ...
            var data = rpQuantityService.doSearchSysPXK60(vm.workItemSearch);
            console.log(data);
            rpQuantityService.doSearchSysPXK60(vm.workItemSearch).then(function (d) {
                CommonService.exportFile(vm.workItemGridSixty, d.data, vm.listRemove, vm.listConvert, "Báo cáo phiếu xuất kho chậm hoàn công 60 ngày");
            });
        }
        vm.listRemove = [{
            title: "Thao tác",
        ...
        vm.patternSignerOptions={
            ...
            dataSource: {
                ...
                transport: {
                    read: function(options) {
                        vm.selectedDept1 = false;
                        return Restangular.all("sysUserCOMSRsService/getForAutoCompleteInSign").post({pageSize:10, page:1, fullName:$("#signerGroupSixty").val().trim()}).then(function(response){
                            options.success(response);
                            if(response == [] || $("#signerGroupSixty").val().trim() == ""){
                                vm.workItemSearch.email = null;
