(function() {
    'use strict';
    var controllerId = 'contructionDTOSController';

    angular.module('MetronicApp').controller(controllerId, contructionDTOSController);

    function contructionDTOSController($scope, $rootScope, $timeout, gettextCatalog,
                                       kendoConfig, $kWindow,$filter,contructionDTOSService,htmlCommonService,
        ...
        var record=0;
        $scope.listCheck = [];
        vm.String = "Quản lý công trình > Quản lý giá trị công trình > Quản lý doanh thu ngoài OS";
        vm.constrObj={};
        ...
        function init(){
            ...
            contructionDTOSService.getAllSourceWork("SOURCE_WORK").then(function(data){
        		vm.sourceWorkDataSource = data.plain();
        		contructionDTOSService.getAllSourceWork("CONSTRUCTION_TYPE").then(function(data){
        ...
        function fillDataTable(){
	        vm.constructionDTOSGridOptions = kendoConfig.getGridOptions({
	            autoBind: true,
	            scrollable: false,
//	            scrollable: {
//                    virtual: true
//                },
				...
				editable: true,
	            save : function(){
	                vm.constructionDTOSGrid.refresh();
	            },
	            ...
	            toolbar: [
	                {
	                    ...
	                    '<label ng-repeat="column in vm.constructionDTOSGrid.columns| filter: vm.gridColumnShowHideFilter">' +
	...
	//                    },
	                ...
	                }, 
	                {
	                    ...
	                    footerTemplate: function(item) {
	                    	var data = vm.constructionDTOSGrid.dataSource.data();
	                    	var item, sum = 0;
...
//	                        }
	                    	if(data.length>0){
	                    		sum = data[0].consAppRevenueValueTotal;
	                    	}
	                        return kendo.toString(sum, "n3");
						},
	                    type :'text',
	                ...
	                },
	                {
	                    ...
	                    template: function(dataItem){
	                    	for(var i=0;i<vm.constructionTypeDataSource.length;i++){
	                    		if(dataItem.constructionTypeNew==null){
	                    			return "";
	                    		}
	                    		if(dataItem.constructionTypeNew==vm.constructionTypeDataSource[i].code){
	                ...
	                '</button>'
	                +'<button style=" border: none; background-color: white;" id=""' +
	                'class=" icon_table" ng-click="vm.cancelQLOS(dataItem)" ng-show="dataItem.consAppRevenueState==1 || dataItem.consAppRevenueState==null"  uib-tooltip="Từ chối" translate' + '>' +
...
vm.cancelComplete = cancelComplete;
 function cancelComplete(obj){
    ...
    if (listId!=null && listId.length>0) {
            var teamplateUrl = "coms/contructionDTOS/cancelCompleteDTOSpopup.html";
...
vm.cancelQLOS = cancelQLOS;
var contrsListDT = [];
function cancelQLOS(dataItem){
// contrsListDT.push(dataItem);
    return Restangular.all("constructionService/checkPermissionsCancel").post(dataItem).then(function (d) {
        ...
        contructionDTOSService.setItem(dataItem);
        var teamplateUrl = "coms/contructionDTOS/cancelDTOSpopup.html";
        var title = "Thông tin hủy bỏ xác nhận";
        var windowId = "CONTRUCTION_DTOS";
...
// contrsListDT.push(dataItem);
    return Restangular.all("constructionService/checkPermissionsCancel").post(dataItem).then(function (d) {
        ...
        contructionDTOSService.setItem(dataItem);
        var teamplateUrl = "coms/contructionDTOS/cancelDTOS2popup.html";
        var title = "Thông tin hủy bỏ xác nhận";
        var windowId = "CONTRUCTION_DTOS";
        CommonService.populatePopupOnPopup(teamplateUrl, title, null, vm, windowId, true, '600', '', 'provinceCode');
    })
...
function getConstructionTask(){
    var list=[];
    var data = vm.constructionDTOSGrid.dataSource._data;
    vm.constructionDTOSGrid.table.find("tr").each(function(idx, item) {
        var row = $(item);
        var checkbox = $('[name="gridcheckbox"]', row);
        if (checkbox.is(':checked')) {
            var tr = vm.constructionDTOSGrid.select().closest("tr");
            var dataItem = vm.constructionDTOSGrid.dataItem(item);
            list.push(dataItem);
        }
    });
    return list;
...
function sumHSHC(){
    var data = vm.constructionOSGrid.dataSource._data;
    var sumHSHC =0;
...
vm.showHideWorkItemColumn = function (column) {
    if (angular.isUndefined(column.hidden)) {
        vm.constructionDTOSGrid.hideColumn(column);
    } else if (column.hidden) {
        vm.constructionDTOSGrid.showColumn(column);
    } else {
        vm.constructionDTOSGrid.hideColumn(column);
    }
...
vm.approveDT =function(dataItem){
    return Restangular.all("constructionService/checkPermissionsApproved").post(dataItem).then(function (d) {
        ...
        contructionDTOSService.setItem(dataItem);
        var obj = contructionDTOSService.getItem();
...
//        hoanm1_20180613_end
        ...
        return Restangular.all("constructionService" + "/updateDTItemApproved").post(obj).then(function (data) {
            ...
            vm.constructionDTOSGrid.dataSource.page(1);
...
vm.rejectionRevenueItem=function(){
	 var obj ={};
	obj.listConsTask=contructionDTOSService.getItem();
...
function refreshGrid(d) {
    var grid = vm.constructionDTOSGrid;
    if (grid) {
...
vm.constructionOSGridOptions= kendoConfig.getGridOptions({
    autoBind: true,
    scrollable: false,
    resizable: true,
    editable: false,
...
function edit(dataItem) {
    ...
    // vm.obj1= obj;
    var teamplateUrl = "coms/contructionDTOS/contructionDTOSpopup.html";
    var title = "Thông tin chi tiết doanh thu ngoài OS";
    var windowId = "CONTRUCTION_DT";
    vm.popupId='EDIT';
...
//tatph-start-3/3/2020
vm.importFile = importFile;
function importFile() {
	var teamplateUrl="coms/contructionDTOS/importFileCons.html";
	var title="Import Doanh thu ngoài OS";
	var windowId="IMPORT_CONSTRUCTION_DT_OS";
...
vm.getExcelTemplateImport = function () {
	 kendo.ui.progress($("#shipment"), true);
		return Restangular.all("recommendContactUnitService/getExcelTemplateConstructionDTOS").post(vm.searchForm).then(function (d) {
...
function submit(){
	...
	if(vm.fileImportData.name.split('.').pop() !=='xls' && vm.fileImportData.name.split('.').pop() !=='xlsx' ){
		...
		return;
	     ...
	     return   $.ajax({
	    	 url: Constant.BASE_SERVICE_URL+"recommendContactUnitService"+"/importConstructionDTOS?folder="+Constant.UPLOAD_FOLDER_TYPE_TEMP,
	            type: "POST",
	            data: formData,
	            enctype: 'multipart/form-data',
	            processData: false,
	            ...
	            if(data == 'NOT_ACCEPTABLE') {
		    		...
		    		vm.disableSubmit = false;
	            		...
	            		$("#constructionDTOSGridId").data('kendoGrid').dataSource.read();
	       			 	$("#constructionDTOSGridId").data('kendoGrid').refresh();
...
vm.exportExcelErr = function(){
	contructionDTOSService.downloadErrorExcel(vm.objectErr).then(function(d) {
		data = d.plain();
