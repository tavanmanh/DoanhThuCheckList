/* Modal Controller */
MetronicApp.controller('comsConstructionSearchController', ['$scope', 'dataTree', 'caller', 'modalInstance1', 'gridOptions', 'popupId', 'isMultiSelect', 'CommonService', 'htmlCommonService', 'SearchService', 'PopupConst', 'RestEndpoint', '$localStorage', '$rootScope', 'Constant', 'gettextCatalog', '$timeout', 
	function ($scope, dataTree, caller, modalInstance1, gridOptions, popupId, isMultiSelect, CommonService, htmlCommonService, SearchService, PopupConst, RestEndpoint, $localStorage, $rootScope, Constant, gettextCatalog, $timeout) {
				...
				dataSource: {
					...
					transport: {
						read: {
							...
							url: Constant.BASE_SERVICE_URL + "constructionTaskService/rpDailyTaskConstruction",
		...
		//VietNT_20181207_start
        function fillTableStationId(result) {
            $scope.gridOptions = kendoConfig.getGridOptions({
                autoBind: true,
                scrollable: false,
                resizable: true,
                ...
                dataSource: {
                    ...
                    transport: {
                        read: {
                            url: Constant.BASE_SERVICE_URL + "constructionTaskService/getConstructionByStationId",
                            contentType: "application/json; charset=utf-8",
                            type: "POST"
                        },
                        parameterMap: function (options, type) {
                            ...
                            if (caller.assignHandover) {
                                obj.catStationId = caller.assignHandover.catStationId;
                            }
		...
		function save(dataItem) {
			if (dataItem) {
				caller.onSaveComsConstruction(dataItem);
				htmlCommonService.dismissPopup();
			}
		}
