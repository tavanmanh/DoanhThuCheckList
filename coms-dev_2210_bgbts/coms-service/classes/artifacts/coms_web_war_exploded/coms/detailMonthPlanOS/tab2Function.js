 ...
 */
function initTab2OSFunction($scope, $rootScope, $timeout, gettextCatalog,
...
//        chinhpxn20180710_end
    ...
    vm.editTab2 = function (dataItem,e) {
        ...
        var templateUrl = "coms/detailMonthPlanOS/popup-edit-tab2.html";
    ...
    vm.editTab3 = function (dataItem,e) {
        ...
        var teamplateUrl = "coms/detailMonthPlanOS/popup-edit-tab3.html";
    ...
    vm.editTab5 = function (dataItem) {
        ...
        var teamplateUrl = "coms/detailMonthPlanOS/popup-edit-tab5.html";
    ...
    vm.indexTab7 = null;
    vm.editTab7 = function (dataItem,e) {
        var grid = vm.rentGroundGrid;
        var row = $(e.target).closest("tr");
        vm.indexTab7 = row.index();
        var templateUrl = "coms/detailMonthPlanOS/popup-edit-thue-tram.html";
        var title = "Thông tin chi tiết thuê trạm";
        var windowId = "THUE_TRAM";
        vm.thueTramItem = angular.copy(dataItem);
        CommonService.populatePopupCreate(templateUrl, title, vm.thueTramItem, vm, windowId, false, '1000', '600', "quantity");

    }
    
    vm.fillDataRentGroundGrid = function fillDataRentGroundGrid() {
    	//Thuê mặt bằng trạm
        vm.rentGroundGridOptions = kendoConfig.getGridOptions({
            autoBind: true,
            scrollable: false,
            resizable: true,
            editable: false,
            ...
            dataSource: {
                ...
                transport: {
                    ...
                    parameterMap: function (options, type) {
                        vm.searchFormRent.page = options.page;
                        vm.searchFormRent.pageSize = options.pageSize;
                        vm.searchFormRent.detailMonthPlanId = vm.detailMonthPlan.detailMonthPlanId;
                        vm.searchFormRent.type = "4";
                        return JSON.stringify(vm.searchFormRent);
...
//                },
                ...
                },
                {
                    ...
                    + '<button style=" border: none; background-color: white;" id="updateId" ng-hide="dataItem.status==0" ng-click="vm.editTab7(dataItem,$event)" class=" icon_table "' +
                    '   uib-tooltip="Sửa" translate>' +
                    '<i class="fa fa-list-alt" style="color:#e0d014" ng-hide="dataItem.status==0"   aria-hidden="true"></i>' +
                    '</button>'
                    +
		        	'		<button ng-show="!!dataItem.completeDate" style=" border: none; background-color: white;" class=" icon_table" uib-tooltip="Khóa" translate> '+
		        	'			<i style="color:grey" class="fa fa-trash" aria-hidden="true"></i>'+
		        	'		</button> '+
		        	'		<button ng-hide="!!dataItem.completeDate" style=" border: none; background-color: white;" ng-click=vm.removeTargetRow(dataItem,$event) class=" icon_table" uib-tooltip="Xóa" translate>'+
...
//                        if (dataItem.workItemType == 1) {
//                            return "Hạng mục thường xuyên";
//                        } else if (dataItem.workItemType == 2) {
//                            return "Hạng mục không thường xuyên";
//                        } else {
//                        	return "";
//                        }
//                    },
...
//                },
                ...
                },
                {
                    ...
                    editable: false,
                    template: function(dataItem){
                    	if(dataItem.sourceWork==1){
                    		return "Xây lắp";
                    	} else if(dataItem.sourceWork==2){
                    		return "Chi phí";
                    	} else if(dataItem.sourceWork==3){
                    		return "Ngoài tập đoàn";
                    	} else if(dataItem.sourceWork==4){
                    		return "Hạ tầng cho thuê xây dựng móng";
                    	} else if(dataItem.sourceWork==5){
                    		return "Hạ tầng cho thuê hoàn thiện";
                    	} else if(dataItem.sourceWork==6){
                    		return "Công trình XDDD";
                    	} else if(dataItem.sourceWork==7){
                    		return "Xây dựng - Trung tâm xây dựng";
                    	} else {
                    		return "";
                    	}
                    }
                },
                {
                    ...
                    editable: false,
                    template: function(dataItem){
                    	if(dataItem.constructionTypeNew==1){
                    		return "Các Công trình BTS, Costie, SWAP và các công trình nguồn đầu tư mảng xây lắp";
                    	} else if(dataItem.constructionTypeNew==2){
                    		return "Các công trình nguồn chi phí";
                    	} else if(dataItem.constructionTypeNew==3){
                    		return "Các công trình Bảo dưỡng ĐH và MFĐ";
                    	} else if(dataItem.constructionTypeNew==4){
                    		return "Các công trình Gpon";
                    	} else if(dataItem.constructionTypeNew==5){
                    		return "Các công trình Hợp đồng 12 đầu việc";
                    	} else if(dataItem.constructionTypeNew==6){
                    		return "Các công trình Ngoài Tập đoàn";
                    	} else if(dataItem.constructionTypeNew==7){
                    		return "Hạ tầng cho thuê";
                    	} else if(dataItem.constructionTypeNew==8){
                    		return "Công trình XDDD";
                    	} else if(dataItem.constructionTypeNew==9){
        ...
        vm.BTSGridOptions = kendoConfig.getGridOptions({
            ...
            dataSource: {
                ...
                transport: {
                    read: {
                        // Thuc hien viec goi service
                        url: Constant.BASE_SERVICE_URL + "DetailMonthPlanOSRsService/doSearchMaterial",
                        contentType: "application/json; charset=utf-8",
                        type: "POST"
                    },
                    parameterMap: function (options, type) {
        ...
        vm.CashFlowGridOptions = kendoConfig.getGridOptions({
            ...
            dataSource: {
                ...
                transport: {
                    read: {
                        // Thuc hien viec goi service
                        url: Constant.BASE_SERVICE_URL + "DetailMonthPlanOSRsService/getRevokeCashMonthPlanByPlanId",
                        contentType: "application/json; charset=utf-8",
                        type: "POST"
                    },
                    parameterMap: function (options, type) {
...
//                },
                {
                    ...
                    template: dataItem =>
                    '<div class="text-center">'
                    + '<button style=" border: none; background-color: white;" id="updateId" ng-hide="dataItem.status==0" ng-click="vm.editRevokeDT(dataItem)" class=" icon_table "' +
                    '   uib-tooltip="Sửa" translate>' +
                    '<i class="fa fa-list-alt" style="color:#e0d014"  ng-hide="dataItem.status==0"   aria-hidden="true"></i>' +
                    '</button>'
                    +
        ...
        vm.dongTienItem = {};
        vm.editRevokeDT = function(dataItem){
        	vm.dongTienItem = angular.copy(dataItem);
            var teamplateUrl = "coms/detailMonthPlanOS/popup-edit-dong-tien.html";
            var title = "Thông tin chi tiết  dòng tiền";
            var windowId = "DONG_TIEN";
            vm.popUpOpen = 'dongTien';
            CommonService.populatePopupCreate(teamplateUrl, title, vm.dongTienItem, vm, windowId, false, '1000', 'auto', "performerId");
...
//                    template:"#= kendo.toString(quantity*1000000, '0.00') #",
                ...
                {
                    title: "Thời gian bắt đầu thi công",
                    width: '10%',
                    field: 'startDate',
                    headerAttributes: {style: "text-align:center;font-weight: bold;"},
                    attributes: {
...
//                    '</button>'
		        	...
		        	'			<i style="color:grey" class="fa fa-trash" aria-hidden="true"></i>'+
		        	'		</button> '+
		        	'		<button ng-hide="vm.showHideButtonRemove(dataItem)" style=" border: none; background-color: white;" ng-click=vm.removeForceMaintainRow(dataItem,$event) class=" icon_table" uib-tooltip="Xóa" translate>'+
		        	'			<i style="color:steelblue;" class="fa fa-trash" aria-hidden="true"></i> '+
		        	'		</button>'
                    + '</div>',
                    width: '10%',
...
//                    '</button>'
		        	...
		        	'			<i style="color:grey" class="fa fa-trash" aria-hidden="true"></i>'+
		        	'		</button> '+
		        	'		<button ng-hide="vm.showHideButtonRemove(dataItem)" style=" border: none; background-color: white;" ng-click=vm.removeSourceRow(dataItem,$event) class=" icon_table" uib-tooltip="Xóa" translate>'+
		        	'			<i style="color:steelblue;" class="fa fa-trash" aria-hidden="true"></i> '+
		        	'		</button>'
                    + '</div>',
                    width: '10%',
...
//    })
confirm('Bạn thật sự muốn xóa bản ghi đã chọn?',function (){
	detailMonthPlanOSService.removeRow(data).then(
		function(d) {
			toastr.success("Xóa thành công!");
...
/**hoangnh 241218 start**/
vm.removeForceMaintainRow = removeForceMaintainRow;
function removeForceMaintainRow(data,e){
		confirm('Bạn thật sự muốn xóa bản ghi đã chọn?',function (){
		debugger;
...
function removeSourceRow(data,e){
	confirm('Bạn thật sự muốn xóa bản ghi đã chọn?',function (){
		debugger;
			if(data.constructionTaskId != null){
				detailMonthPlanOSService.removeRow(data).then(
...
// 8.2 Search SysGroup
    ...
    function importExcelTab2() {
        var teamplateUrl = "coms/detailMonthPlanOS/import-popup.html";
    ...
    vm.getExcelTemplate = function () {
        ...
        } else if (vm.value == THUE_TRAM) {
            obj.request = 'exportTemplateRent';
        }
        
        obj.sysGroupId = vm.detailMonthPlan.sysGroupId;
        detailMonthPlanOSService.downloadTemplate(obj).then(function (d) {
    ...
    //  hungnx 20180618 start
      ...
      //hungnx 20180618 end
        ...
        detailMonthPlanOSService.updateListTC(vm.detailMonthPlan).then(function (response) {
    ...
    vm.updateListHSHC = function () {
      ...
      //hungnx 20180618 end
        ...
        detailMonthPlanOSService.updateListHSHC(vm.detailMonthPlan).then(function (response) {
            toastr.success("Ghi lại thành công!");
            //  hungnx 20180618 start
            vm.loadingUpdate = false;
          //hungnx 20180618 end
    ...
    vm.updateListLDT = function () {
      ...
      //hungnx 20180618 end
        ...
        detailMonthPlanOSService.updateListLDT(vm.detailMonthPlan).then(function (response) {
                toastr.success("Ghi lại thành công!");
                //  hungnx 20180618 start
                vm.loadingUpdate = false;
              //hungnx 20180618 end
    ...
    vm.updateListDT = function () {
      ...
      //hungnx 20180618 end
        ...
        detailMonthPlanOSService.updateListDT(vm.detailMonthPlan).then(function (response) {
            toastr.success("Ghi lại thành công!");
    ...
    vm.updateListCVK = function () {
      ...
      //hungnx 20180618 end
        ...
        detailMonthPlanOSService.updateListCVK(vm.detailMonthPlan).then(function (response) {
            toastr.success("Ghi lại thành công!");
            //  hungnx 20180618 start
            vm.loadingUpdate = false;
          //hungnx 20180618 end
    ...
    vm.updateListDmpnOrder = function () {
      ...
      //hungnx 20180618 end
        ...
        detailMonthPlanOSService.updateListDmpnOrder(vm.detailMonthPlan).then(function (response) {
            toastr.success("Ghi lại thành công!");
            //  hungnx 20180618 start
            vm.loadingUpdate = false;
          //hungnx 20180618 end
        ...
        return $.ajax({
            ...
            success: function (data) {
                if (data.length === 0) {
                	...
                	//tatph-12/9/2019-start - check trung lap con trinh khi import
//                	for(var i = 0 ; i < data.length ; i ++){
//                		$scope["checkDuplicateConstructionCode"+i] = [];
//                		for(var j = 0 ; j < data.length ; j ++){
//                    		if(data[i].workItemName == data[j].workItemName && data[i].constructionCode == data[j].constructionCode){
//                    			$scope["checkDuplicateConstructionCode"+i].push({"constrCode" :data[i].constructionCode,
//                    													"workItem" : data[i].workItemName	});
//                    			if($scope["checkDuplicateConstructionCode"+i].length >=2){
//                    				toastr.error("Công trình :"+data[i].constructionCode+" và hạng mục :"+data[i].workItemName +" đã trùng lặp");
//                       			 return;
//                    			}
//                    			 
//                    		}
...
//    hungnx 20180618 end
        return $.ajax({
            url: Constant.BASE_SERVICE_URL + "DetailMonthPlanOSRsService/importHSHC",
            type: "POST",
            data: formData,
            enctype: 'multipart/form-data',
            processData: false,
...
//      hungnx 20180618 end
        return $.ajax({
            url: Constant.BASE_SERVICE_URL + "DetailMonthPlanOSRsService/importLDT",
            type: "POST",
            data: formData,
            enctype: 'multipart/form-data',
            processData: false,
...
//      hungnx 20180618 end
        return $.ajax({
            url: Constant.BASE_SERVICE_URL + "DetailMonthPlanOSRsService/importCVK",
            type: "POST",
            data: formData,
            enctype: 'multipart/form-data',
            processData: false,
...
//      hungnx 20180618 end
        return $.ajax({
            url: Constant.BASE_SERVICE_URL + "DetailMonthPlanOSRsService/importThuHoiDT",
            type: "POST",
            data: formData,
            enctype: 'multipart/form-data',
            processData: false,
...
//      hungnx 20180618 end
        return $.ajax({
            url: Constant.BASE_SERVICE_URL + "DetailMonthPlanOSRsService/importYCVT",
            type: "POST",
            data: formData,
            enctype: 'multipart/form-data',
            processData: false,
...
//          hungnx 20180618 end
    ...
    function getTotalMonthPlanDetail() {
        var teamplateUrl = "coms/detailMonthPlanOS/total-month-plan-detail-os-form.html";
    ...
    $scope.$on("Popup.open", function () {
        if (vm.popUpOpen == 'detailMonth') {
            detailMonthPlanOSService.getYearPlanDetailTarget(vm.detailMonthPlan).then(function (result) {
            ...
            detailMonthPlanOSService.getWorkItemDetail(vm.hoanCongItem).then(function (result) {
                //var grid = $("#yearPlanDetail").data("kendoGrid");
            ...
            detailMonthPlanOSService.getWorkItemDetail(vm.doanhThuItem).then(function (result) {
                //var grid = $("#yearPlanDetail").data("kendoGrid");
            ...
            detailMonthPlanOSService.getWorkItemDetail(vm.dongTienItem).then(function (result) {
                //var grid = $("#yearPlanDetail").data("kendoGrid");
    ...
    function fillTotalMonthPlanDetailTable() {
        vm.planDetailOSGridOptions = kendoConfig.getGridOptions({
            autoBind: true,
            scrollable: false,
            editable: false,
            resizable: true,
...
//        var quan = vm.thiCongItem.quantity.replace(/\,/g,'');
        ...
        dataItem.quantity =vm.thiCongItem.quantity;
...
//save popup doanh thu
    ...
    function saveDoanhThuPopup() {
        ...
        /*detailMonthPlanOSService.updateDT(vm.doanhThuItem).then(function(result){
...
//save popup dong tien
  ...
  vm.saveDongTienPopup = function(){
	  var grid = $("#CashFlow").data("kendoGrid").dataSource.data();
	  for(var i=0;i<grid.length;i++){
		  if(grid[i].billCode==vm.dongTienItem.billCode){
			  grid[i].createdBillDate = vm.dongTienItem.createdBillDate;
			  grid[i].billValue = vm.dongTienItem.billValue;
		  }
	  }
	  
	  $("div.k-window-actions > a.k-window-action > span.k-i-close").click();
	  toastr.success("Cập nhật thành công");
	  $("#CashFlow").data("kendoGrid").refresh();
  }
...
//detail-CashFlow-tab5-end
    ...
    vm.importExcelTab7 = importExcelTab7;
    function importExcelTab7() {
        var teamplateUrl = "coms/detailMonthPlanOS/import-popup.html";
        var title = "Import từ file excel";
        var windowId = "DETAIL_MONTH_PLAN_RENT";
        CommonService.populatePopupCreate(teamplateUrl, title, null, vm, windowId, true, '700', 'auto', "code");
    }
    
    function importRentGround(formData) {
        $('#savepopupMonth').prop('disabled', true);
    	htmlCommonService.showLoading('#loadingIpMonth');
        return $.ajax({
            url: Constant.BASE_SERVICE_URL + "DetailMonthPlanOSRsService/importRentGround",
            type: "POST",
            data: formData,
            enctype: 'multipart/form-data',
            processData: false,
...
//                        vm.detailMonthPlan.isHSHCImport = 1;
    ...
    vm.saveThueTramPopup = saveThueTramPopup;
    function saveThueTramPopup() {
        if (vm.thueTramItem.performerId == null) {
            toastr.warning("Người thực hiện không được để trống")
            return;
        } 
        else if (vm.thueTramItem.startDate == undefined || vm.thueTramItem.startDate == '') {
            toastr.warning("Thời gian bắt đầu không được để trống")
            return;
        }
        else if (vm.thueTramItem.endDate == undefined || vm.thueTramItem.endDate == '') {
            toastr.warning("Thời gian kết thúc không được để trống")
            return;
        }
        vm.cancelEditPopup();
        toastr.success("Ghi lại thành công");
        var list= vm.rentGroundGrid.dataSource.data();
        var dataItem = list[vm.indexTab7];
//        var quan = vm.thiCongItem.quantity.replace(/\,/g,'');
        dataItem.catProvinceCode = vm.thueTramItem.catProvinceCode;
        dataItem.catStationId = vm.thueTramItem.catStationId;
        dataItem.stationCode = vm.thueTramItem.stationCode;
        dataItem.completeDate = vm.thueTramItem.completeDate;
        dataItem.performerId = vm.thueTramItem.performerId;
        dataItem.performerName = vm.thueTramItem.performerName;
        dataItem.startDate = vm.thueTramItem.startDate;
        dataItem.endDate = vm.thueTramItem.endDate;
        dataItem.description = vm.thueTramItem.description;
        vm.rentGroundGrid.refresh();
