(function () {
    ...
    function assignHandoverController($scope, $templateCache, $rootScope, $timeout, gettextCatalog,
        ...
        var vm = this;
        vm.String = "Quản lý công trình > Quản lý công trình > Giao việc cho CN/TTKV";
        vm.searchForm = {
            status: 1
        };
...
//            edit();
        ...
        vm.assignHandoverGridOptions = kendoConfig.getGridOptions({
            ...
            toolbar: [
                {
                    name: "actions",
                    template:
                        '<button class="btn btn-qlk padding-search-right addQLK ng-scope" ng-click="vm.add()" uib-tooltip="Tạo mới" translate="">Tạo mới</button>'+
            ...
            dataSource: {
                ...
                transport: {
                    read: {
                        url: Constant.BASE_SERVICE_URL + RestEndpoint.ASSIGN_HANDOVER_SERVICE_URL + "/doSearch",
            ...
            columns: [
                ...
                +
                '<button class="btn btn-qlk padding-search-right ng-scope" id="attachEdit" ng-disabled="!!!dataItem.isDesign"' +
                'ng-click="vm.attachFileEdit(dataItem)" uib-tooltip="Chọn file thiết kế" translate="" ' +
                'list-file-type="doc, docx, pdf">Chọn file</button>'
                +
                '<button style=" border: none; background-color: white;"' +
                'class="#=appParamId# icon_table"  ng-hide="dataItem.status==0"  ng-click="vm.remove(dataItem)"  uib-tooltip="Xóa" translate>' +
...
//            }
        ...
        function exportFile() {
			...
			Restangular.all("assignHandoverService" + "/doSearch").post(data).then(function(d) {
                var data = d.data;
                for (var i = 0; i < data.length; i++) {
...
//                    : "";
				...
				CommonService.exportFile(vm.assignHandoverGrid, d.data, listRemove, listConvert, "Danh sách Giao việc");
			});
        }
