/* Modal Controller */
MetronicApp.controller('staffSearchController', ['$scope', 'dataTree', 'caller', 'modalInstance1', 'gridOptions', 'popupId', 'isMultiSelect', 'CommonService', 'htmlCommonService', 'SearchService', 'PopupConst', 'RestEndpoint', '$localStorage', '$rootScope', 'Constant', 'gettextCatalog', '$timeout', 
				...
				dataSource: {
					...
					transport: {
						read: {
							...
							url: Constant.BASE_SERVICE_URL + "configStaffTangentRestService/catProvince/doSearchStaffByPopup",
		...
		function save(dataItem) {
			if (dataItem) {
				if($scope.popupId=='searchForm'){
					caller.onSaveStaffSearch(dataItem);
					htmlCommonService.dismissPopup();
				} else if($scope.popupId=='solution') {
					caller.onSaveStaffSolution(dataItem);
					htmlCommonService.dismissPopup();
				} else {
