(function () {
    'use strict';
    var controllerId = 'yearPlanController';

    angular.module('MetronicApp').controller(controllerId, yearPlanController);

    function yearPlanController($scope, $rootScope, $timeout, gettextCatalog,
                                kendoConfig, $kWindow, yearPlanService,
                ...
                dataSource: {
                    ...
                    transport: {
                        read: {
                            ...
                            url: Constant.BASE_SERVICE_URL + "yearPlanRsService/doSearch",
                        ...
                        parameterMap: function (options, type) {
                            ...
                            vm.yearPlanSearch.page = options.page
                            vm.yearPlanSearch.pageSize = options.pageSize

                            return JSON.stringify(vm.yearPlanSearch)
        ...
        function chkSelectAll() {
            ...
            if (vm.chkAll) {
                if ($scope.checkSearch && $scope.dataSearch.length > 0) {
                    ...
                    CommonService.getallData("yearPlanRsService/doSearch", vm.yearPlanSearch).then(function (data) {
        ...
        // return
        // Restangular.all("yearPlanRsService/exportYearPlan").post().then(function
        // (d) {
        // var data = d.plain();
        ...
        vm.exportFile = function () {
        	function displayLoading(target) {
        	      ...
        	      setTimeout(function(){
        	    	  ...
        	    	  return Restangular.all("yearPlanRsService/exportYearPlan").post(vm.yearPlanSearch).then(function (d) {
        	                var data = d.plain();
        ...
        vm.import = function () {
            var teamplateUrl = "coms/yearPlan/yearPlanPopup.html";
...
//          hungnx 20180618 end
        ...
        function add() {
            ...
            yearPlanService.getSequence({}).then(function (seq) {
        ...
        function edit(dataItem) {
            vm.String = "Quản lý công trình > Quản lý kế hoạch > Chi tiết kế hoạch năm";
            vm.showDetail = true;
            vm.yearPlan = dataItem;
            vm.isDisabled = dataItem.signState != 1;
            yearPlanService.getYearPlanById(dataItem.yearPlanId).then(function (data) {
                vm.yearPlan = data;
        ...
        function copy(dataItem) {
            vm.String = "Quản lý công trình > Quản lý kế hoạch > Sao chép kế hoạch năm";
            vm.showDetail = true;
            vm.yearPlan = dataItem;
            yearPlanService.getYearPlanById(dataItem.yearPlanId).then(function (data) {
                vm.yearPlan = data;
...
// validate mỗi đơn vị phải đủ 12 tháng
        ...
        function remove(dataItem) {
            confirm('Bạn thật sự muốn xóa bản ghi đã chọn?', function () {
                yearPlanService.remove(dataItem).then(
                    function (d) {
...
//hungtd_20181213_start
        ...
        function updateRegistry(dataItem) {
            confirm('Bạn thật sự muốn trình ký bản ghi đã chọn?', function () {
                yearPlanService.updateRegistry(dataItem).then(
                    function (d) {
...
//        hungtd_20181213_end
        ...
        vm.getExcelTemplate = function () {
            yearPlanService.downloadTemplate({}).then(function (d) {
                var data = d.plain();
                window.location = Constant.BASE_SERVICE_URL + "fileservice/downloadFileATTT?" + data.fileName;
            }).catch(function () {
                toastr.error(gettextCatalog.getString("Lỗi khi tải biểu mẫu!"));
        ...
        function sendToSign(dataItem) {
            if (dataItem.signState == 1 || dataItem.signState == 2) {
                ...
                obj.listSignVoffice = [];
                var dataList = [];
                Restangular.all("yearPlanRsService/getAppParamByType").post('YEAR_PLAN_VOFFICE').then(function (data) {
                    var i = 1;
        ...
        function cancelSingState() {
            vm.yearPlanSearch.signStateList = []
        }
