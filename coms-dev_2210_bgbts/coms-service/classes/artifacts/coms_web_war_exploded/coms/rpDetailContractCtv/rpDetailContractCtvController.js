(function() {
	'use strict';
	var controllerId = 'rpDetailContractCtvController';

	angular.module('MetronicApp').controller(controllerId,
			rpDetailContractCtvController);

	function rpDetailContractCtvController($scope, $rootScope, $timeout,
			gettextCatalog, kendoConfig, $kWindow, rpDetailContractCtvService,
		...
		vm.showHideWorkItemColumn = function(column) {
			if (angular.isUndefined(column.hidden)) {
				vm.rpDetailContractCtvGrid.hideColumn(column);
			} else if (column.hidden) {
				vm.rpDetailContractCtvGrid.showColumn(column);
			} else {
				vm.rpDetailContractCtvGrid.hideColumn(column);
			}
		...
		function doSearch(){
			...
			var grid = vm.rpDetailContractCtvGrid;
			if(grid){
		...
		function fillDataTable(data) {
			vm.rpDetailContractCtvGridOptions = kendoConfig.getGridOptions({
				autoBind : true,
				scrollable : true,
				resizable : true,
				editable : false,
				...
				toolbar : [ {
					...
					template : '<div class=" pull-left ">'
							...
							+ '<label ng-repeat="column in vm.rpDetailContractCtvGrid.columns.slice(1,vm.rpDetailContractCtvGrid.columns.length)| filter: vm.gridColumnShowHideFilter">'
				...
				dataSource : {
					...
					schema : {
						total : function(response) {
							$("#appCountRpDetailContractCtv").text(
									"" + response.total);
							vm.countRpDetailContractCtv = response.total;
					...
					transport : {
						read : {
							...
							url : Constant.BASE_SERVICE_URL
									+ "rpBTSService/doSearchRpDetailContractCtv",
				...
				columns : [
						...
						{
							title : "Thông tin đại lý/cộng tác viên",
							width : '300px',
							headerAttributes : {
								style : "text-align:center;font-weight: bold;"
							},
						...
						},
						{
							...
							columns : [
								...
								{
									title : "Giá trị trước thuế của HĐ thi công",
									width : '100px',
									field : "giaTruocThue",
									headerAttributes : {
										style : "text-align:center;font-weight: bold;"
									...
									format: '{0:n3}',
									footerTemplate: function(item) {
				                        var data = vm.rpDetailContractCtvGrid.dataSource.data();
				                        if(data.length>0){
...
//								},
		...
		vm.exportFile = function(){
			...
			kendo.ui.progress($("#rpDetailContractCtvGrid"), true);
			vm.searchForm.thang = $("#dateDetail").val();
			rpDetailContractCtvService.exportRpDetailContractCtv(vm.searchForm).then(function(d){
				var data = d.plain();
        	    window.location = Constant.BASE_SERVICE_URL + "fileservice/downloadFileATTT?" + data.fileName;
        	    kendo.ui.progress($("#rpDetailContractCtvGrid"), false);
			}).catch(function (e) {
				kendo.ui.progress($("#rpDetailContractCtvGrid"), false);
