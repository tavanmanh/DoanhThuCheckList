...
// BUILD LOCAL
var API_URL="/coms-service/";


// BUILD TEST OLD
// var API_URL="http://10.58.71.134:9752/coms-service/";
//var API_URL="http://10.58.71.134:8452/coms-service/";
...
MetronicApp.run(["$location","$rootScope", "settings", "$state",  "Idle","Restangular","Constant", function($location,$rootScope, settings, $state, Idle,Restangular,Constant) {
   ...
   Restangular.setErrorInterceptor(function(response, deferred, responseHandler) {
    ...
    if($rootScope.authenticatedUser == 'undefined' ||$rootScope.authenticatedUser==null){
        var paramValue = $location.search().user;
    	 // Restangular.one(Constant.authen.LOGIN_URL).get().then(function(user){
    	 Restangular.one("auth/kttsAuthenRsService/login2?user="+paramValue).get().then(function(user){
    		 ...
    		 if($rootScope.casUser!=null){
    			 Restangular.one(Constant.authen.GET_USER_INFO).get().then(function(user){    	    	
                      ...
                      $rootScope.vpsInfo = user.VpsUserInfo;
...
//    	url common add and update for DetailMonthPlanRsService/add, DetailMonthPlanRsService/update, totalMonthPlanRsService, yearPlanRsService
    	var ListKeyUrl=[
//    	                total month plan page
    	                'saveAppendixFile','saveContract','saveFinance','saveMaterial','saveForceNew','saveForceMaintain'
    	                ,'saveSource','saveTarget','saveForceNewBTS','totalMonthPlanRsService/add', 'totalMonthPlanRsService/update'
//    	                detail month plan
    	                ,'updateListTC', 'updateListHSHC','updateListLDT', 'DetailMonthPlanRsService/update'
    	                , 'updateListDmpnOrder','updateListDT', 'updateListCVK', 'DetailMonthPlanRsService/add'
//    	                year plan
    	                ,'yearPlanRsService/update', 'yearPlanRsService/add'
//    	                'importMaterial','importForceNewLine','importForceNewBts','importContract','importFinance','importForceMaintain','importCVK','importDT','importYCVT','importHSHC','importLDT'
//    	                ,'importTC','importYearPlanDetail','exportPDFConstructionReport','exportExcelConstructionReport', 'exportExcelTemplate','signVoffice',"?folder=", "importFileThauPhu"
    	                ];
	...
	function enterPass(){
		...
		.open({
			...
			template: "<div class='col-md-12 form-group'>" +
						...
						"<form><input type = 'password' class='form-control' name='vofficePass' id='vofficePass' ng-model='pass'/>" +
						"<input type = 'checkbox' ng-click='showPassword()' /> Hiện mật khẩu" +
						"</form>"+
...
//			$("div.k-window-actions > a.k-window-action > span.k-i-close").click();
	...
	function showPassword(){
		var x = document.getElementById("vofficePass");
		if(x.type =="password"){
			x.type = "text";
		} else {
			x.type = "password";
