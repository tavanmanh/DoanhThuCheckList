...
public class AttachmentDAO extends BaseFWDAOImpl<AttachmentBO, Long> {
    ...
    public List<AttachmentDTO> doSearch(AttachmentDTO obj) {
        StringBuilder sql = new StringBuilder("SELECT"
                ...
                + " JOIN CAT_OWNER.APP_PARAM APP ON APP.CODE=WA.APP_PARAM_CODE"
                + " WHERE 1=1 ");
    ...
    //HuyPQ-start
    @SuppressWarnings("unchecked")
	public List<AttachmentDTO> doSearchFileTk(AttachmentDTO obj) {
    	StringBuilder sql = new StringBuilder(
				"SELECT" + " WA.UTIL_ATTACH_DOCUMENT_ID utilAttachDocumentId," + " WA.OBJECT_ID objectId,"
						+ " WA.FILE_PATH filePath," + " WA.TYPE type," + " WA.APP_PARAM_CODE appParamCode,"
						+ " WA.CODE code," + " WA.NAME name," + " WA.ENCRYT_NAME encrytName,"
						+ " WA.DESCRIPTION description," + " WA.STATUS status, " + " APP.NAME appParamName,APP.PAR_ORDER parOrder "
						+ " FROM UTIL_ATTACH_DOCUMENT WA" + " JOIN APP_PARAM APP ON APP.CODE=WA.APP_PARAM_CODE"
						+ " WHERE 1=1 AND APP.par_type = 'FILE_TYPE' ");

			sql.append(" AND WA.OBJECT_ID =:objectId ");
			sql.append(" AND WA.TYPE in ('58','59') ");
			sql.append(" AND WA.STATUS = 1 ");
			sql.append(" ORDER BY WA.CODE");
		SQLQuery query = getSession().createSQLQuery(sql.toString());
		...
		List<AttachmentDTO> ls = query.list();
