...
public class StockDAO extends BaseFWDAOImpl<StockBO, Long> {
    ...
    public List<StockDTO> doSearch(StockDTO obj) {
        StringBuilder sql = new StringBuilder("SELECT ST.STOCK_ID stockId," + "ST.CODE code," + "ST.NAME name,"
                ...
                + "ST.LEVEL_STOCK levelST" + ", AP.NAME nameStock, AP1.NAME levelStock" + " FROM CAT_OWNER.STOCK ST JOIN CAT_OWNER.APP_PARAM AP "
                + "ON(ST.TYPE = AP.CODE AND AP.PAR_TYPE like 'STOCK_TYPE') JOIN CAT_OWNER.APP_PARAM AP1 ON(ST.LEVEL_STOCK = AP1.CODE and AP1.PAR_TYPE like 'STOCK_LEVEL')  WHERE 1=1");
