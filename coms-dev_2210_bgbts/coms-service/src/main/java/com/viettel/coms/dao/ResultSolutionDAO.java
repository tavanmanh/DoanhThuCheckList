...
@Transactional
@Repository("resultSolutionDAO")
public class ResultSolutionDAO extends BaseFWDAOImpl<ResultSolutionBO, Long> {

	public ResultSolutionDAO() {
		this.model = new ResultSolutionBO();
	}

	public ResultSolutionDAO(Session session) {
		this.session = session;
	}

	public List<ResultSolutionDTO> getAllContractXdddSuccess(ResultSolutionDTO obj, Long sysGroupId) {
		/*StringBuilder sql = new StringBuilder("SELECT CNT_CONTRACT_ID contractId, " + "  C.code contractCode, "
				...
				// lamdt18-Mã CTV trên IMS thuộc kênh XHH
				"  AND C.channel              =2 "
				+ "  AND C.COLLABORATORS_ID IN ( SELECT SYS_USER_ID FROM SYS_USER WHERE TYPE_USER ='1' ) " +
				// lamdt18
				// lamdt18-Mã Mã CTV tiếp xúc có trong CTV trên IMS:
				// lamdt18
			...
			if(obj.getSource() == null || !obj.getSource().equals("3") ) {
				sql.append("AND SYS_GROUP_ID= (select case when sys.group_level=4 then  (select sys_group_id from sys_group a where a.sys_group_id=  " + 
				...
				sql.append(" AND c.COLLABORATORS_ID in (select Created_user from TANGENT_CUSTOMER) ");
			}*/
			StringBuilder sql = new StringBuilder(" SELECT ");
			sql.append(" CNT_CONTRACT_ID contractId, C.code contractCode, C.SIGN_DATE signDate, C.PRICE contractPrice, ");
			sql.append(" C.WARRANTY_NUMBER_DAY guaranteeTime, C.DEPLOYMENT_DATE constructTime ");
			sql.append(" FROM cnt_contract C ");
			sql.append(" WHERE C.CONTRACT_TYPE =0 AND C.status! =0   AND C.CONTRACT_TYPE_O =4 ");
			sql.append(" AND CNT_CONTRACT_APPROVE =1  ");
			sql.append(" AND CNT_CONTRACT_ID NOT IN (SELECT NVL(CONTRACT_ID,0) FROM RESULT_SOLUTION ) ");
			if(obj.getSource().equals("1") || obj.getSource().equals("2")) {
				if(obj.getSource().equals("1")) {
					sql.append(" AND C.channel = 1 AND C.USER_SEARCH_ID IN ( SELECT SYS_USER_ID FROM SYS_USER WHERE TYPE_USER IS NULL ) ");
					sql.append(" AND C.USER_SEARCH_ID in (select Created_user from TANGENT_CUSTOMER) ");
				}else {
					sql.append(" AND C.channel =2   AND C.COLLABORATORS_ID IN ( SELECT SYS_USER_ID FROM SYS_USER WHERE TYPE_USER ='1' ) ");
					sql.append(" AND c.COLLABORATORS_ID in (select Created_user from TANGENT_CUSTOMER) ");
				}
				sql.append(" AND SYS_GROUP_ID= (select case when sys.group_level=4 then  (select sys_group_id from sys_group a where a.sys_group_id= ");
				sql.append(" (select parent_id from sys_group a where a.sys_group_id=sys.parent_id)) ");
				sql.append(" when sys.group_level=3 then ");
				sql.append(" (select sys_group_id from sys_group a where a.sys_group_id=sys.parent_id)   else sys_group_id ");
				sql.append(" end sys_group_id   from sys_group sys ");
				sql.append(" where sys_group_id = :sysGroupId) ");
			}
		if (StringUtils.isNoneBlank(obj.getContractCode())) {
			sql.append(" AND upper(CODE) like upper(:code) escape '&' ");
		}
		// lamdt18- Ngày tạo tiếp xúc của kênh XHH > Ngày ký của Hợp đồng XDDD tại IMS
		...
		if(obj.getSource() != null && obj.getSource().equals("1") || obj.getSource().equals("2")) {
	...
	public ResultSolutionDTO getResultSolutionByContractId(Long contractId) {
		...
		List<ResultSolutionDTO> ls = query.list();
		if (ls.size() > 0) {
			return ls.get(0);
		}
		return null;
	...
	public List<ResultSolutionDTO> getResultSolutionByTangentCustomerId(Long customerId) {
		StringBuilder sql = new StringBuilder("select ");
		sql.append("RESULT_SOLUTION_ID resultSolutionId, ").append("TANGENT_CUSTOMER_ID tangentCustomerId, ")
				.append("RESULT_SOLUTION_ORDER resultSolutionOrder, ")
				.append("PRESENT_SOLUTION_DATE presentSolutionDate, ")
	...
	public ResultSolutionDTO getResultSolutionJoinSysUserByResultSolutionId(Long solutionId) {
		StringBuilder sql = new StringBuilder("select ");
		sql.append("rs.RESULT_SOLUTION_ID resultSolutionId, ").append("rs.TANGENT_CUSTOMER_ID tangentCustomerId, ")
				.append("rs.RESULT_SOLUTION_ORDER resultSolutionOrder, ")
				.append("rs.PRESENT_SOLUTION_DATE presentSolutionDate, ")
	...
	public List<ResultSolutionBO> getResultSolutionByTangentCustomerId(ResultSolutionDTO obj) {
	...
	public ResultSolutionDTO getResultSolutionFieldByTargentCustomerId(Long tangentCustomerId) {
		StringBuilder sql = new StringBuilder("select ");
		sql.append("RESULT_SOLUTION_ID resultSolutionId, ").append("TANGENT_CUSTOMER_ID tangentCustomerId, ")
				...
				.append("FROM RESULT_SOLUTION where TANGENT_CUSTOMER_ID = :tangentCustomerId ");
		sql.append(" ORDER BY RESULT_SOLUTION_ID DESC FETCH FIRST 1 ROW ONLY ");
		SQLQuery query = getSession().createSQLQuery(sql.toString());
	...
	public List<ResultSolutionDTO> getAllContractXdddSuccessInternal(ResultSolutionDTO obj, Long sysGroupId){
