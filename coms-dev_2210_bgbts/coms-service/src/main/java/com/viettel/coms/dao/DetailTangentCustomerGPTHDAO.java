...
@Transactional
@Repository("detailTangentCustomerGPTHDAO")
public class DetailTangentCustomerGPTHDAO extends BaseFWDAOImpl<DetailTangentCustomerGPTHBO, Long> {

	public DetailTangentCustomerGPTHDAO() {
		this.model = new DetailTangentCustomerGPTHBO();
	}

	public DetailTangentCustomerGPTHDAO(Session session) {
	...
	public DetailTangentCustomerGPTHDTO getDetailsTangentCustomer(TangentCustomerGPTHDTO obj) {
		...
		if (null == obj.getPartnerType()) {
			...
			StringBuilder sql = new StringBuilder(
					"SELECT T1.DETAIL_TANGENT_CUSTOMER_GPTH_ID detailTangentCustomerGPTHId,T1.TANGENT_CUSTOMER_ID tangentCustomerId,T1.PARTNER_TYPE partnerType, " +
			...
			sql.append(" FROM DETAIL_TANGENT_CUSTOMER_GPTH T1 WHERE 1 = 1 ");
