...
@Transactional
@Repository("resultTangentGPTHDAO")
public class ResultTangentGPTHDAO extends BaseFWDAOImpl<ResultTangentGPTHBO, Long>{

	public ResultTangentGPTHDAO() {
		this.model = new ResultTangentGPTHBO();
	}

	public ResultTangentGPTHDAO(Session session) {
		this.session = session;
	}
	
	public List<ResultTangentGPTHBO> getResultTangentByTangentCustomerId(ResultTangentGPTHDTO obj){
		StringBuilder sql = new StringBuilder(" SELECT * from RESULT_TANGENT_GPTH where TANGENT_CUSTOMER_GPTH_ID=:tangentId ");
		SQLQuery query = getSession().createSQLQuery(sql.toString());
		query.addEntity(ResultTangentGPTHBO.class);
		query.setParameter("tangentId", obj.getTangentCustomerGPTHId());
		return query.list();
	}
	
	public ResultTangentGPTHDTO getMaxAppointmentDateByTangentCustomerId(ResultTangentGPTHDTO obj){
		StringBuilder sql = new StringBuilder(" SELECT T1.APPOINTMENT_DATE appointmentDate,T1.RESULT_TANGENT_GPTH_ID resultTangentGPTHId,"
				...
				+ " from RESULT_TANGENT_GPTH T1 ");
		sql.append(" where 1 = 1 and T1.TANGENT_CUSTOMER_GPTH_ID =:tangentId ORDER BY T1.RESULT_TANGENT_GPTH_ID DESC FETCH FIRST 1 ROW ONLY ");
		SQLQuery query = getSession().createSQLQuery(sql.toString());
		...
		List<ResultTangentGPTHDTO> ls = query.list();
		if(ls.size()>0) {
			return ls.get(0);
		}
		return null;
	...
	public List<ResultTangentGPTHDTO> getListResultTangentByTangentCustomerId(ResultTangentGPTHDTO obj){
	...
	public ResultTangentGPTHDTO getListResultTangentJoinSysUserByResultTangentId(ResultTangentGPTHDTO obj){
		StringBuilder sql = new StringBuilder(" SELECT ");
		sql.append("rt.RESULT_TANGENT_ID resultTangentId, ")
		.append("rt.TANGENT_CUSTOMER_ID tangentCustomerId, ")
		.append("rt.APPOINTMENT_DATE appointmentDate, ")
		...
		query.setParameter("id", obj.getResultTangentGPTHId());
	...
	public ResultTangentGPTHDTO getResultTangentTypeByResultTangentId(Long id){
		StringBuilder sql = new StringBuilder(" SELECT ");
		sql.append("rt.RESULT_TANGENT_TYPE resultTangentType ")
		.append("FROM RESULT_TANGENT_GPTH rt ")
		
		.append("WHERE 1=1 AND rt.RESULT_TANGENT_GPTH_ID =:id ");
		sql.append(" ORDER BY rt.ORDER_RESULT_TANGENT DESC ");
		SQLQuery query = getSession().createSQLQuery(sql.toString());
