...
//    	  System.out.println("===11==============");
    ...
    @SuppressWarnings("unchecked")
	public List<EstimatesWorkItemsDTO> doSearchByWorkItems(ConstrWorkLogsDTO dto) {
	String sql = "SELECT DISTINCT ccw.WORK_ITEM_TYPE_NAME workItemTypeName, "
			+ " ccw.WORK_ITEM_CODE itemsCode,"
			+ " ccw.WORK_ITEM_NAME itemName,"
			+ " ccw.CAT_TASK_CODE workItemCode,"
			+ " ccw.CAT_TASK_NAME workItemName,"
			+ " wial.ACCEPTANCE_DATE acceptanceDate, "
			+ " wih.PRIORITY_WORK_ITEM, " + 
			"  cth.PRIORITY_TASK "
			+ " FROM WORK_ITEMS_ACCEPT_LIST wial LEFT JOIN CNT_CONSTR_WORK_ITEM_TASK_HCQT ccw"
			+ " ON wial.WORK_ITEM_ID = ccw.WORK_ITEM_ID"
			+ " AND wial.ESTIMATES_WORK_ITEM_ID = ccw.CAT_TASK_ID"
			+ " LEFT JOIN WORK_ITEMS_ACCEPTANCE wia ON wial.WORK_ITEMS_ACCEPTANCE_ID = wia.WORK_ITEMS_ACCEPTANCE_ID"
			+ " left join WORK_ITEM_HCQT wih on wial.WORK_ITEM_ID = wih.WORK_ITEM_ID " + 
			" left join CAT_TASK_HCQT cth on wial.ESTIMATES_WORK_ITEM_ID = cth.CAT_TASK_ID "
			+ " WHERE wia.CONSTRUCT_ID = :constructId AND wial.ACCEPTANCE_DATE <= :logDate AND wia.IS_ACTIVE !=0 ";
		
		sql = sql + " ORDER BY wih.PRIORITY_WORK_ITEM," + 
				"  to_number(cth.PRIORITY_TASK) ASC ";
		
		StringBuilder sqlBuilder = new StringBuilder(sql);
		SQLQuery query = getSession().createSQLQuery(sqlBuilder.toString());
		query.addScalar("itemsCode", new StringType());
		...
		query.setResultTransformer(Transformers.aliasToBean(EstimatesWorkItemsDTO.class));
			query.setParameter("constructId", dto.getConstructId());
			query.setParameter("logDate", dto.getLogDate());	
		return query.list();
	}
