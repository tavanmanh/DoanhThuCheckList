    ...
    public Long updateDataTable(ConstrWorkLogsDTO obj) {
        StringBuffer sql = new StringBuffer(
                "update CONSTR_WORK_Logs CW set CW.LOG_DATE =:logDate,CW.WORK_CONTENT =:workContent,"
                        ...
                        + "CW.A_MONITOR_ID =:aMonitorId, CW.B_CONSTRUCT_ID =:bConstructId " 
                        + " ,CW.CONSTRUCTION_CONDITION=:constructionCondition, CW.WORKER_COUNT= :workerCount " //Huypq-20190821-add
                        + "where constr_work_logs_id =:constrWorkLogsId ");
        SQLQuery query = getSession().createSQLQuery(sql.toString());
        ...
        query.setParameter("constructionCondition", obj.getConstructionCondition());
		query.setParameter("workerCount", obj.getWorkerCount());
        query.executeUpdate();
