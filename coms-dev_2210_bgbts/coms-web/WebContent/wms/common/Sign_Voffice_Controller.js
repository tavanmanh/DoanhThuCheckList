...
MetronicApp.controller('signVofficeController', [
		...
		function($scope,businessType, caller, modalInstance,data, popupId, isMultiSelect,
				...
				$localStorage, $rootScope,Restangular,$http) {
			...
			function onRowChange(dataItem){debugger;
				if (dataItem) {
...
//					$scope.list=dataItem.listSignVoffice;
			...
			function preview() {
				...
				var msg=null;
				if(rowList.length==0){
					toastr.error("Không có dữ liệu để xem trước!");
						CommonService.dismissPopup();
					return;
				}
			CommonService.previewVoffice(rowList[0]);

//download phu luc thang chi tiet - tam thoi supspend
            if('KeHoachThangChiTiet'==rowList[0].reportName) {
                CommonService.downloadAttachFile(rowList[0]);
            }
}
