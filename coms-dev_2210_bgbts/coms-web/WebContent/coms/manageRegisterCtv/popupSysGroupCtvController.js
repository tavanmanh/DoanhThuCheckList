/* Modal Controller */
MetronicApp.controller('popupSysGroupCtvController', [
		'$scope',
		'dataTree',
		'caller',
		'modalInstance1',
		...
		$( document ).ready(function() {
			...
			if(popupId=="province"){
				...
				obj.sysGroupId = caller.provinceIdXddd;
			}
			
			if($('#treeviewUnits')){
				CommonService.getSysGroupCtv(obj).then(function(result){
					var data=genData(result.data,null);
					setupTree(data);
			...
			function onChangeTree(dataItem){
				...
				obj.id=dataItem.id
				 ...
				 CommonService.getSysGroupCheckTTKTDV(obj).then(function(result){
		 ...
		 setTimeout(function(){
			...
			function save(dataItem) {
				if(popupId=="province"){
					caller.onSaveGroupProvince(dataItem);
				} else if(popupId=="sysGroup"){
					caller.onSaveSysGroup(dataItem);
				}
				cancel();
			}
...
//			 $("#filterText").keyup(function (e) {
			...
			$scope.doSearch= function(){
				...
				if(popupId=="province"){
					$scope.searchGrid.sysGroupId = null;
				} else {
					$scope.searchGrid.sysGroupId = caller.provinceIdXddd;
				}
				CommonService.getSysGroupCtv($scope.searchGrid).then(function(result){
					var grid =	$scope.gridView;
