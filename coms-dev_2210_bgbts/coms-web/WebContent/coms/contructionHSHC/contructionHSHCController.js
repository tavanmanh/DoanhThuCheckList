(function() {
    'use strict';
    var controllerId = 'contructionHSHCController';

    angular.module('MetronicApp').controller(controllerId, contructionHSHCController);
    function contructionHSHCController($scope, $rootScope, $timeout, gettextCatalog,contructionHSHCService,
        ...
        var record=0;
        $scope.listCheck = [];
        vm.String = "Quản lý công trình > Quản lý giá trị công trình > Quản lý HSHC";
        vm.constrObj={};
        vm.constrTaskObj={};
...
//                	 title:"Ngày nhận HSHC",
//                	 field: 'receiveRecordsDate',
//                	 template:  "<input id='ngayNhanHshc' kendo-date-picker  ng-model='dataItem.receiveRecordsDate' ng-change='vm.updateDayHshc(dataItem)' style='width: 90px;' min-year='1000' date-time />",
//                	 editor: function(container,options){
//                		 var $input = $("<input/>").attr("receiveRecordsDate",options.field).appendTo(container);
//                		 $input.datepicker({dateFormat:"MM/dd/yyyy",});
//                	 },
//                 	format:"dd/MM/yyyy",
//                 	 width: '100px',
//                     headerAttributes: {style: "text-align:center;font-weight: bold;"},
...
//       	    var d = new Date();  
     	...
     	var dateNowNext1 = kendo.parseDate(currMonthNextDate1,"dd/MM/yyyy");
            return Restangular.all("constructionTaskService/checkPermissionsApprovedHSHC").post(dataItem).then(function (data) {
            	...
            	//hoanm1 if(monthDateNow<12 && dateNowCurr > dateNowNext){
            	
            	//hoanm1	  toastr.error(gettextCatalog.getString("Đã quá thời gian phê duyệt HSHC"));
            	
            	//hoanm1  }else if(monthDateNow = 12 && dateNowCurr > dateNowNext1) {
            	//hoanm1	  toastr.error(gettextCatalog.getString("Đã quá thời gian phê duyệt HSHC"));
        ...
        vm.undoHSHC = function(dataItem){
               ...
               dataItem.sysUserId = Constant.userInfo.vpsUserToken.sysUserId;
               if(dataItem.completeUserUpdate != dataItem.sysUserId) {
...
vm.handleCheck = handleCheck;
        function handleCheck(dataItem) {
            ...
            var yearDateNow = dateCheck.getFullYear();
            debugger;
           //hoanm1 if(monthDateNow<12){
           //hoanm1 	var currMonthNext = monthDateNow+1;
           //hoanm1 	var currMonthNextDate = '05' + '/' + currMonthNext +'/'+ yearDateNow;
           //hoanm1 	var dateNowNext = kendo.parseDate(currMonthNextDate,"dd/MM/yyyy");
            //hoanm1		if(dateNowCurr > dateNowNext){
            //hoanm1		toastr.warning("Đã quá thời gian phê duyệt!");
           //hoanm1 		return;
           //hoanm1 	}
           //hoanm1 } else{
           //hoanm1 	var currMonthNext = '01';
           //hoanm1 	var currYearNext = yearDateNow + 1;
            //hoanm1	var currMonthNextDate = '05' + '/' + currMonthNext +'/'+ currYearNext;
           //hoanm1 	var dateNowNext = kendo.parseDate(currMonthNextDate,"dd/MM/yyyy");
           //hoanm1 	if(dateNowCurr > dateNowNext){
            //hoanm1		toastr.warning("Đã quá thời gian phê duyệt!");
            //hoanm1		return;
...
//	hungtd_20181211_end
    ...
    vm.removeComplete = function(dataItem){
        ...
        return Restangular.all("constructionTaskService/checkPermissions").post(dataItem).then(function (data) {
        	...
        	var templateUrl = "coms/contructionHSHC/popupCancelConfirm.html";
...
function getConstruction(){
    ...
    return list;
      ...
      //HuyPQ-25/08/2018-edit-start
        ...
        function doSearch() {
        	//console.log(vm.constructionTaskGrid.dataSource);
            //vm.constructionTaskGrid.dataSource.page(1);
            var grid = vm.constructionTaskGrid;
            if (grid) {
...
vm.onSelectBGMBFile = function(e) {
    ...
    return   $.ajax({
    	...
    	function importConstructionHSHC() {
            ...
            var templateUrl = "coms/contructionHSHC/importConstructionHSHC.html";
            var title = "Import Hồ sơ hoàn công";
            var windowId = "IMPORT_CONSTRUCTION_HSHC";
        ...
        function submitConstructionHSHC() {
            ...
            // start importing
            var formData = new FormData();
            var strMonthYear = kendo.toString(vm.searchForm.monthYear, "MM/yyyy");
            formData.append('multipartFile', vm.fileImportData);
            return $.ajax({
                url: Constant.BASE_SERVICE_URL + "constructionTaskService/importConstructionHSHC?folder=temp&monthYear=" + strMonthYear,
                type: "POST",
                data: formData,
                enctype: 'multipart/form-data',
                processData: false,
        ...
        vm.exportExcelErr = function () {
        	contructionHSHCService.downloadErrorExcel(vm.objectErr).then(function (d) {
                var data = d.plain();
                window.location = Constant.BASE_SERVICE_URL + "fileservice/downloadFileATTT?" + data.fileName;
            }).catch(function () {
