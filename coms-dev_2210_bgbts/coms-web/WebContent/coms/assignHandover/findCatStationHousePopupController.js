...
MetronicApp.controller('findCatStationHousePopupController', ['$scope', 'dataTree', 'caller', 'modalInstance1', 'gridOptions', 'popupId', 'isMultiSelect', 'CommonService', 'htmlCommonService', 'SearchService', 'PopupConst', 'RestEndpoint', '$localStorage', '$rootScope', 'Constant', 'gettextCatalog', '$timeout',
	function ($scope, dataTree, caller, modalInstance1, gridOptions, popupId, isMultiSelect, 
			CommonService, htmlCommonService, SearchService, PopupConst, RestEndpoint, $localStorage, $rootScope, Constant, gettextCatalog, $timeout) {
		...
		$(document).ready(function () {
			fillTable();
			$timeout(function (){angular.element('#stationHouseCode').focus();}, 500);
		});
				...
				dataSource: {
					...
					transport: {
						read: {
							...
							url: Constant.BASE_SERVICE_URL + "constructionService/getStationHouseForAutoComplete",
		...
		function save(dataItem) {
			if (dataItem) {
				caller.onSaveCatStationHouse(dataItem);
				htmlCommonService.dismissPopup();
//	            if(document.getElementById("stationHouse")) {
//					document.getElementById("stationHouse").focus();
//				}
			}
		}
