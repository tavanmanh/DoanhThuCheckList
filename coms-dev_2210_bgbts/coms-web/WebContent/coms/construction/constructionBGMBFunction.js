 ...
 */
function constructionBGMBFunction($scope, $rootScope, $timeout, gettextCatalog,$filter,
    ...
    function updateBGMBItem(){
        if(vm.constrObj.handoverDate==undefined||vm.constrObj.handoverDate==''){
            toastr.warning("Ngày bàn giao MB không được để trống!")
            return;
        }
        if(vm.BGMBgrid!=undefined && vm.BGMBgrid.dataSource!=undefined)
            vm.constrObj.listFileBGMB = vm.BGMBgrid.dataSource._data
        ...
        constructionService.updateBGMBItem(vm.constrObj).then(function(data){
            if (data.error) {
    ...
    $scope.onSelectBGMBFile = function(e) {
        ...
        return   $.ajax({
            ...
            success:function(data) {
                ...
                $("#fileNameBGMB")[0].innerHTML=$("#fileBGMB")[0].files[0].name
            }
        });
    }
    function refreshBGMBGrid(data,grid) {
        if(grid){
    ...
    vm.BGMBgridOptions = kendoConfig.getGridOptions({
        ...
        columns: [
            ...
            'class=" icon_table" ng-click="vm.removeBGMBItemFile($event)"  uib-tooltip="Xóa" translate' + '>' +
