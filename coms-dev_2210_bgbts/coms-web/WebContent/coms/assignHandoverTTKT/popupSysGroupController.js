/* Modal Controller */
MetronicApp.controller('popupSysGroupTTKTController', [
		'$scope',
		'dataTree',
		'caller',
		'modalInstance1',
		...
		$( document ).ready(function() {
		 ...
		 setTimeout(function(){
			...
			caller.onSaveTTKTDV(dataItem);
				...
				if ($scope.dataItem) {
				caller.onSaveTTKTDV($scope.dataItem, $scope.popupId);
				CommonService.dismissPopup1();
				} else {
					if($scope.parent){
...
//			 $("#filterText").keyup(function (e) {
			...
			$scope.doSearch= function(){
				...
				CommonService.getSysGroupCheckTTKTDV($scope.searchGrid).then(function(result){
					var grid =	$scope.gridView;
					grid.dataSource.data(result.plain());
					if(result.plain().length>0){
						grid.dataSource.page(1);
