(function() {
	'use strict';
	var controllerId = 'detailMonthPlanController';
	
	angular.module('MetronicApp').controller(controllerId, detailMonthPlanController);
	
	function detailMonthPlanController($scope, $rootScope, $timeout, gettextCatalog, 
			kendoConfig, $kWindow,detailMonthPlanService,$filter,
		...
		// Khoi tao du lieu cho form
		 initTab2Function($scope, $rootScope, $timeout, gettextCatalog,
    ...
    vm.exportExcelGrid= function(){
    	function displayLoading(target) {
			...
			setTimeout(function(){
				...
				return Restangular.all("DetailMonthPlanRsService/exportDetailMonthPlan").post(vm.detailMonthPlanSearch).then(function (d) {
			        var data = d.plain();
    ...
    vm.exportExcelTab1= function(){
    	function displayLoading(target) {
			...
			setTimeout(function(){
				...
				return Restangular.all("DetailMonthPlanRsService/exportDetailMonthPlanTab1").post( vm.searchForm).then(function (d) {
		            var data = d.plain();
		            window.location = Constant.BASE_SERVICE_URL + "fileservice/downloadFileATTT?" + data.fileName;
		            kendo.ui.progress(element, false);
		        }).catch(function (e) {
    ...
    vm.exportExcelTab2= function(){
    	function displayLoading(target) {
			...
			setTimeout(function(){
				...
				return Restangular.all("DetailMonthPlanRsService/exportDetailMonthPlanTab2").post( vm.searchFormHSHC).then(function (d) {
		            var data = d.plain();
		            window.location = Constant.BASE_SERVICE_URL + "fileservice/downloadFileATTT?" + data.fileName;
		            kendo.ui.progress(element, false);
		        }).catch(function (e) {
    ...
    vm.exportExcelTab3= function(){
    	function displayLoading(target) {
			...
			setTimeout(function(){
				...
				return Restangular.all("DetailMonthPlanRsService/exportDetailMonthPlanTab3").post( vm.searchFormForceMainTain).then(function (d) {
		            var data = d.plain();
		            window.location = Constant.BASE_SERVICE_URL + "fileservice/downloadFileATTT?" + data.fileName;
		            kendo.ui.progress(element, false);
		        }).catch(function (e) {
    ...
    vm.exportExcelTab4= function(){
    	function displayLoading(target) {
			...
			setTimeout(function(){
				...
				return Restangular.all("DetailMonthPlanRsService/exportDetailMonthPlanBTS").post( vm.searchFormBTS).then(function (d) {
		            var data = d.plain();
		            window.location = Constant.BASE_SERVICE_URL + "fileservice/downloadFileATTT?" + data.fileName;
		            kendo.ui.progress(element, false);
		        }).catch(function (e) {
    ...
    vm.exportExcelTab5= function(){
    	function displayLoading(target) {
			...
			setTimeout(function(){
				...
				return Restangular.all("DetailMonthPlanRsService/exportDetailMonthPlanTab5").post( vm.searchFormCashFlow).then(function (d) {
		            var data = d.plain();
		            window.location = Constant.BASE_SERVICE_URL + "fileservice/downloadFileATTT?" + data.fileName;
		            kendo.ui.progress(element, false);
		        }).catch(function (e) {
    ...
    vm.exportExcelTab6= function(){
    	function displayLoading(target) {
			...
			setTimeout(function(){
				...
				return Restangular.all("DetailMonthPlanRsService/exportDetailMonthPlanTab6").post( vm.searchFormOtherJobs).then(function (d) {
		            var data = d.plain();
		            window.location = Constant.BASE_SERVICE_URL + "fileservice/downloadFileATTT?" + data.fileName;
		            kendo.ui.progress(element, false);
		        }).catch(function (e) {
...
vm.handleCheck=handleCheck;
		 ...
		 function chkSelectAll(){
		     ...
		     if(vm.chkAll){
		         if( $scope.checkSearch && $scope.dataSearch.length >0){
		             ...
		             CommonService.getallData("DetailMonthPlanRsService/doSearch",vm.detailMonthPlanSearch).then(function(data){
		...
		vm.add=add;
		function add(){
            return Restangular.all("DetailMonthPlanRsService/checkPermissionsAdd").post().then(function (d) {
                if(d.error){
                ...
                vm.showDetail = true;
                vm.sysGroupCode = '';
                vm.String = "Quản lý công trình > Quản lý kế hoạch > Thêm mới kế hoạch tháng chi tiết";
                var year = (new Date()).getFullYear();
                ...
                detailMonthPlanService.getSequence({}).then(function (seq) {
                    vm.seq = seq;
		...
		function edit(dataItem){
            ...
            initTab2Table();
            detailMonthPlanService.getById(dataItem.detailMonthPlanId).then(function(result){
                vm.String="Quản lý công trình > Quản lý kế hoạch > Chỉnh sửa kế hoạch tháng chi tiết";
                vm.showDetail = true;
                ...
                return Restangular.all("DetailMonthPlanRsService/checkPermissionsUpdate").post(dataItem).then(function (d) {
                    if(d==2){
		...
		function copy(dataItem){
            return Restangular.all("DetailMonthPlanRsService/checkPermissionsCopy").post().then(function (d) {
                if(d.error){
                    toastr.error(d.error);
                    return;
                }
...
//              hungnx 20180619 end
		        if (vm.detailMonthPlan.detailMonthPlanId == null || vm.detailMonthPlan.detailMonthPlanId == '') {
		            ...
		            detailMonthPlanService.createdetailMonthPlan(data).then(function (result) {
		                if (result.error) {
		                    $('#year').focus();
...
//                      hungnx 20180619 end
 //                       vm.cancelAdd()
		            ...
		            detailMonthPlanService.updatedetailMonthPlan(data).then(function (result) {
		                if (result.error) {
		                    $('#year').focus();
		                    toastr.error(result.error);
//	                      hungnx 20180619 start
...
function cancelAdd(){
    ...
    vm.String="Quản lý công trình > Quản lý kế hoạch > Kế hoạch tháng chi tiết";
		...
		function remove(dataItem){
            return Restangular.all("DetailMonthPlanRsService/checkPermissionsDelete").post(dataItem).then(function (d) {
                if(d.error){
                    toastr.error(d.error);
                    return;
                }
...
// var dataItem = grid.dataSource.getByUid(uid);
                confirm('Bạn thật sự muốn xóa bản ghi đã chọn?', function () {
                    detailMonthPlanService.remove(dataItem).then(
                        function (data) {
...
// var dataItem = grid.dataSource.getByUid(uid);
                confirm('Bạn thật sự muốn trình ký bản ghi đã chọn?', function () {
                    detailMonthPlanService.updateRegistry(dataItem).then(
                        function (d) {
...
//		hungtd_20181213_end
		...
		vm.gridColumnShowHideFilter = function (item) { 
            ...
            vm.exportpdf= function(){
            	var obj={};
            	detailMonthPlanService.exportpdf(obj);
            }
        ...
        function sendToSign(dataItem) {
            return Restangular.all("DetailMonthPlanRsService/checkPermissionsRegistry").post().then(function (d) {
                if(d.error){
                    toastr.error(d.error);
                    return;
                }
