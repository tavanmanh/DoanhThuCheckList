...
angular.module('MetronicApp').factory('totalMonthPlanOSService', ['$http', '$q', 'RestEndpoint', 'Restangular', '$kWindow', function($http, $q, RestEndpoint, Restangular, $kWindow){
    var serviceUrl = 'totalMonthPlanOSRsService';
    var factory = {
        remove:remove,
    ...
    function downloadTemplate(obj) {
     ...
     function getById(id) {
        return Restangular.all(serviceUrl+"/getByIdV2").post(id);
    }
     
     function getListTmpn(id) {
         return Restangular.all(serviceUrl+"/getListTmpn").post(id);
     }
