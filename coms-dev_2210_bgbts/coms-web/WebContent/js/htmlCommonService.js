...
//	        	hungnx 20180618 start
				...
				//picasso start
				saveFile:saveFile,
				apWorkSrcShortName:apWorkSrcShortName,
				//picasso end
	    };
...
//	    hungnx 070618 end
			...
			modalInstance1 = $kWindow
					.open({
						options : {
							...
							close : function() {
								// modalInstance = null;
								//caller.closeForm(); // PhucPV edit 09072020
								$rootScope
		...
		function apWorkSrcShortName(name){
			 var splitted = name.split("-");
			 if(splitted[1]){
			 	var firstDashIndex = name.indexOf('-');
			 	return name.slice(firstDashIndex+1).trim();
			 }
